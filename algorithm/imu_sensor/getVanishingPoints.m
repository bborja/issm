function [horizon_params, R] = getVanishingPoints(B, imu_pitch_initial, imu_roll_initial, fr_num, path, fs, seq_id, varargin)
    %% PATHS
    path_IMU = fullfile(path, 'imu');
    imu_pitch_initial = imu_pitch_initial + varargin{1};
    imu_roll_initial = imu_roll_initial + varargin{2};
    %% Load IMU measurements offset from calisbration sequence
    ...
    % Build rotation matrix from imu measurements (Euler angles)
