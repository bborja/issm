function [ detector_state, sel_xy, objects, masked_sea, Q_sum_large ] = detect_edge_of_sea_simplified_ISSM(detector_state, im, p_factor, scaled_image_size, varargin)
    ...
    for i = 1 : 2 : num_argin
        switch varargin{i}
            case 'prior_sea'
                priors_sea = varargin{i+1};
                args_issm{counter_args_issm} = varargin{i};
                args_issm{counter_args_issm+1} = varargin{i+1};
                counter_args_issm = counter_args_issm + 2;

            case 'prior_sky'
                priors_sky = varargin{i+1};
                args_issm{counter_args_issm} = varargin{i};
                args_issm{counter_args_issm+1} = varargin{i+1};
                counter_args_issm = counter_args_issm + 2;

            case 'avg_hor_y'
                avg_hor_centery = varargin{i+1};

            case 'hiper_fix'
                hiper_priors_fix = varargin{i+1};

            case 'hor_angle'
                hor_angle = varargin{i+1};
        end
    end
...
%        assert(isequal(em_image_size, scaled_image_size), 'Change this if EM image size is not equal to specified size');
        ...
        %Update hiper-priors using IMU informations...
        if( exist('hor_angle', 'var') && exist('hiper_priors_fix', 'var') && exist('avg_hor_centery', 'var')) 
            % calculate new hipper-priors
            [tmp_mus, tmp_cov_landcomp] = improveHiperPriors([detector_state.prior_mixture(1).Mu'; ...
															  detector_state.prior_mixture(2).Mu'; ...
															  detector_state.prior_mixture(3).Mu'], ...
														      detector_state.prior_mixture(2).Cov, ...
															  avg_hor_centery, hiper_priors_fix, hor_angle, scaled_image_size);
            
            % update hiperpriors with new values
            for i = 1 : 3
                detector_state.prior_mixture(i).Mu = tmp_mus(i,:)';
            end
			
            detector_state.prior_mixture(2).Cov = tmp_cov_landcomp;
			
			clear tmp_mus tmp_cov_landcomp
        end
    ...
    if(exist('args_issm', 'var'))
        [detector_state, Q_sum_large] = run_ISSM(detector_state, dataEM, p_factor, args_issm{:});
    else
        [detector_state, Q_sum_large] = run_ISSM(detector_state, dataEM, p_factor);
    end
...
%--------------------------------------------------------------------------
function [ detector_state, Q_sum_large ] = run_ISSM(detector_state, data, p_factor, varargin)
    num_argin = numel(varargin);
    ...
    p_unknown = getUnknownWeightForTheFeatureModel(detector_state.colorspace, detector_state.em_image_size, ...
												   detector_state.use_uniform_component, p_factor);
    ...
    p = zeros(mixture_size+1,size(data,2)) ;
     ...
     while cont > 1
		...
		% Apply mask on priors and posteriors
		if(exist('priors_sea', 'var'))
			PI_i(:, :, 3) = PI_i(:, :, 3) .* priors_sea';
		end
		
		if(exist('priors_sky', 'var'))
			PI_i(:, :, 1) = PI_i(:, :, 1) .* priors_sky';
		end
...
function p_unknown = getUnknownWeightForTheFeatureModel(type_colorspace, sizeMask, use_uniform_component, p_factor) 
     %#codegen
